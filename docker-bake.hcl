...
target "hardened" {
    tags = ["docker.io/niklastremmel/mitarbeiter:2026.10.19-hardened"]
    #dockerfile = "Dockerfile"
    #no-cache = true
}

target "trixie" {
    tags = ["docker.io/niklastremmel/mitarbeiter:2026.10.19-trixie"]
    dockerfile = "Dockerfile.trixie"
    #no-cache = true
}

target "alpine" {
    tags = ["docker.io/niklastremmel/mitarbeiter:2026.10.19-alpine"]
