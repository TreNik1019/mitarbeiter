...
Das Basis-Verzeichnis `mitarbeiter` (direkt unter `src`) wird durch `__init__.py` als Modul
für das Projekt deklariert, damit `mitarbeiter` als oberster Namensraum bei den Imports
verwendet werden kann.

Außerdem kann mit dem Package Manager _uv_ das Modul als Skript aufgerufen werden:
`uv run mitarbeiter`. Siehe `project.scripts` in `pyproject.toml`.

Desweiteren wird für _uvicorn_ die 'Package-Level' Variable `app` initialisiert.
Dann lautet der Aufruf:

```powershell
uv run uvicorn src.mitarbeiter:app --ssl-certfile=src\mitarbeiter\config\resources\tls\certificate.crt --ssl-keyfile=src\mitarbeiter\config\resources\tls\key.pem
```
...
uvicorn src.mitarbeiter:app --ssl-certfile=src\mitarbeiter\config\resources\tls\certificate.crt --ssl-keyfile=src\mitarbeiter\config\resources\tls\key.pem
```
...
```powershell
uv run fastapi dev src\mitarbeiter
```
"""  # noqa: E501

from mitarbeiter.asgi_server import run
from mitarbeiter.fastapi_app import app
